...
Splunk.Module.Redirector = $.klass(Splunk.Module, {
    ...
    stripLastStandardReportCommand: function(s) {
        var commands = Sideview.utils.getCommands(s);
     ...
     */
    mergeDrilldownKeys: function(context, argList) {
    ...
    getNewSearchTerms: function(context, p, matchedXField, matchedSplitByField) {
        var newTerms = [];
        ...
        if (!matchedXField && context.has(p + ".xTerm") && context.has(p + ".xFilter")) {
            ...
            if (context.get("sideview.xFieldBins")>0) {
                ...
                try {
                    ...
                    newTerms.push(Sideview.utils.getSearchTermsFromFilters(xFilters));
                    xFilters[0]["operator"] = "<";
                    xFilters[0]["value"] = xTermMinMax[1];
                    newTerms.push(Sideview.utils.getSearchTermsFromFilters(xFilters));
                } 
                catch(e) {
                    this.logger.error("unexpected exception while trying to parse a bucketed xFilter argument - " + str(e));
        ...
        if (!matchedSplitByField && context.has(p + ".splitByTerms") && context.has(p + ".splitByFilters")) {
            if (context.get("sideview.zFieldBins")>0) {
                if (context.get("sideview.zFieldBins")>0) {
                    try {
                        var splitByFilters = JSON.parse(context.get(p + ".splitByFilters"));
                        var splitByTermMinMax=splitByFilters[0]["value"].split("-");
                        splitByFilters[0]["operator"] = ">=";
                        splitByFilters[0]["value"] = splitByTermMinMax[0];
                        newTerms.push(Sideview.utils.getSearchTermsFromFilters(splitByFilters));
                        splitByFilters[0]["operator"] = "<";
                        splitByFilters[0]["value"] = splitByTermMinMax[1];
                        newTerms.push(Sideview.utils.getSearchTermsFromFilters(splitByFilters));
                    } 
                    catch(e) {
                        this.logger.error("unexpected exception while trying to parse a bucketed splitByFilter argument - " + str(e));
    ...
    combineTerms: function(currentTerms, newTerms) {
        var commands = Sideview.utils.getCommands(currentTerms);
    ...
    processArgs: function(context, argList) {
        ...
        if (this.getParam("mergeDrilldownKeys")=="True") {
        ...
        for (var key in argList) {
            ...
            var substitutedArgKey = Sideview.utils.replaceTokensFromContext(key, context);
            ...
            if (singleKeyName && context.get(singleKeyName)) {
                ...
                var value = Sideview.utils.getValueForFormElementSelection(singleKeyName,context);
            ...
            // It has either static components, or it's more than one key
                ...
                var substitutedArgValue = Sideview.utils.replaceTokensFromContext(argValue, context);
    ...
    addArgs: function(url, args) {
        ...
        var argString = Sideview.utils.dictToString(args);
        if (argString) {
            url += "?" + argString;
        }
        return url;
    ...
    getURL: function(context) {
        ...
        var url = Sideview.utils.replaceTokensFromContext(this.getParam("url"), context);
    ...
    onContextChange: function() {
        ...
        if (this.getParam("target")) {
            ...
            var newWindow = context.get("click.modifierKey") || Sideview.utils.normalizeBoolean(this.getParam("popup"), true);
            if (newWindow) {
                window.open(url, "_blank", explicitWindowFeatures || "resizable=yes,status=no,scrollbars=yes,toolbar=no");
            } else {
                if (explicitWindowFeatures) {
                    this.logger.warn("Redirector module is configured with explicit windowFeatures param, although neither the popup param nor the target param is set. This may be a view-configuration error.");
                }
